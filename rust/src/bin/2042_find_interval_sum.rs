...
impl SegmentTree {
    ...
    fn build(&mut self, node: usize, s: usize, e: usize) -> i64 {
        ...
        let m = (s + e) / 2;
        self.tree[node] = self.build(2 * node, s, m) + self.build(2 * node + 1, m + 1, e);
        self.tree[node]
    }

    fn sum(&mut self, l: usize, r: usize) -> i64 {
        self.sum_dfs(1, l - 1, r - 1, 0, self.n - 1)
    }

    fn sum_dfs(&mut self, node: usize, l: usize, r: usize, s: usize, e: usize) -> i64 {
        if l > e || r < s { return 0; }
        if l <= s && e <= r { return self.tree[node]; }
        let m = (s + e) / 2;
        self.sum_dfs(2 * node, l, r, s, m) + self.sum_dfs(2 * node + 1, l, r, m + 1, e)
    }

    fn update(&mut self, idx: usize, new_val: i64) {
        let idx = idx - 1;
        let dif = new_val - self.arr[idx];
        self.arr[idx] = new_val;
        self.update_dfs(idx, dif, 1, 0, self.n - 1);
    }

    fn update_dfs(&mut self, idx: usize, dif: i64, node: usize, s: usize, e: usize) {
        if idx < s || e < idx { return; }
        self.tree[node] += dif;
        if s == e { return; }
        let m = (s + e) / 2;
        self.update_dfs(idx, dif, 2 * node, s, m);
        self.update_dfs(idx, dif, 2 * node + 1, m + 1, e);
    }
}
    ...
    for _ in 0..(m + k) {
        ...
        match a {
            ...
            2 => {
                ...
                let p_sum = seg_tree.sum(b, c);
                writeln!(out, "{p_sum}").unwrap();
            },
            _ => unreachable!(),
        }
    }
