...
fn main() {
    ...
    let (k, n) = (scan.token::<usize>(), scan.token::<u32>());

    let mut graph = vec![vec![]; 2 * k + 1];
    for _ in 0..n {
        let (l1, c1) = (scan.token::<usize>(), scan.token::<char>());
        let (l2, c2) = (scan.token::<usize>(), scan.token::<char>());
        let (l3, c3) = (scan.token::<usize>(), scan.token::<char>());
        let (l1, l2, l3) = (
            if c1 == 'R' { l1 + k } else { l1 },
            if c2 == 'R' { l2 + k } else { l2 },
            if c3 == 'R' { l3 + k } else { l3 },
        );
        graph[not(l1, k)].push(l2);
        graph[not(l2, k)].push(l1);
        graph[not(l2, k)].push(l3);
        graph[not(l3, k)].push(l2);
        graph[not(l3, k)].push(l1);
        graph[not(l1, k)].push(l3);
    }

    let mut scc = SCC::new(2 * k);
    scc.tarjan(&graph);

    for i in 1..=k {
        if scc.index[i] == scc.index[not(i, k)] {
            println!("-1");
            return;
        }
    }

    let mut t_order = Vec::with_capacity(2 * k);
    for i in 1..=(2 * k) {
        t_order.push((scc.index[i], i));
    }
    t_order.sort_unstable_by(|a, b| b.cmp(a));

    let mut ans = vec!['A'; k + 1];
    for &(_, cur) in t_order.iter() {
        let (idx, is_red) = if cur > k { (cur - k, true) } else { (cur, false) };
        if ans[idx] == 'A' {
            ans[idx] = if is_red { 'B' } else { 'R' };
        }
    }
