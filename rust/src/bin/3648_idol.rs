...
fn main() {
    ...
    while let Some(n) = scan.token::<usize>() {
        let m = scan.token::<u32>().unwrap();

        let mut graph = vec![vec![]; 2 * n + 1];
        // Add -1 to 1 to force Sanggeun be true.
        graph[not(1, n)].push(1);
        for _ in 0..m {
            let (from, to) = (scan.token::<i32>().unwrap(), scan.token::<i32>().unwrap());
            let (from, to) = (
        ...
        writeln!(out, "{}", if is_possible { "yes" } else { "no "}).unwrap();
    }
}
...
mod scanner {
    ...
    impl <R: io::BufRead> UnsafeScanner<R> {
        ...
        pub(crate) fn token<T: str::FromStr>(&mut self) -> Option<T> {
            loop {
